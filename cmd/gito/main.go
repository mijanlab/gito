...
func printVersion() {
	fmt.Printf("gito current version: v%s\n", strings.TrimPrefix(version, "v"))

	// Check if a newer version is available
	rel, hasUpdate, err := updater.CheckLatestVersion(version)
	if err == nil && rel != nil && hasUpdate {
		fmt.Printf("Available Version:    %s\n", rel.TagName)
		fmt.Printf("Run 'gito update' to upgrade to the latest version.\n")
	}
}

func runUpdate() {
	fmt.Println("◈ Checking for Gito updates...")
...
func runUninstall() {
	...
	fmt.Print("Are you sure you want to uninstall Gito? [y/N]: ")
...
func printHelp() {
	fmt.Println(`Gito — Premium Interactive Git TUI & Fast CLI
...
Commands:
  ...
  uninstall        Remove Gito binary from your system (use --purge to remove config)
  version          Show current version and check for available updates

Flags:
  -y, --yes        Auto-accept AI commit message on push
  -v, --version    Show Gito current version
